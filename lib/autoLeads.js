...
async function autoDiscoverLeads({ brancheQueries, postnummer, perBranche = 15 }, employee) {
  ...
  return {
    ...
    // Kun dem hvor der faktisk blev fundet en side. Tidligere blev tallet
    // regnet som "alle minus dem der blev til leads", og da leads nu først
    // opstår når køen er tømt, kom de uafklarede til at tælle som "har side".
    withWebsite: checked.filter((c) => c.check.found).length,
    ...saveResult,
  };
}
