...
function getSavedCvrNumbers() {
  const db = readDb();
  return [
    ...(db.leads || []).map((l) => String(l.cvrNummer)),
    ...(db.leadVerify || []).map((l) => String(l.cvrNummer)),
    ...(db.leadsWithWebsite || []).map((l) => String(l.cvrNummer)),
  ];
}
...
function queueForVerification(results) {
  return mutate((db) => {
    ...
    for (const r of results) {
      // CVR leverer nummeret som tal; gemmes som tekst, så det matcher det
      // der senere kommer ind fra en indsat liste.
      const cvr = String(r.cvrNummer);
      if (known.has(cvr)) continue;
      db.leadVerify.push({
        id: nanoid(),
        cvrNummer: cvr,
        navn: r.navn,
...
// Tømmer køen uden at gemme noget. Virksomhederne glemmes helt og kan
// dukke op igen i en senere søgning — modsat at afgøre dem, hvor de enten
// bliver til leads eller markeres som havende en hjemmeside.
function clearVerificationQueue() {
  return mutate((db) => {
    const count = (db.leadVerify || []).length;
    db.leadVerify = [];
    return count;
  });
}
...
function applyVerification(verdicts, savedBy = "automatisk verifikation") {
  return mutate((db) => {
    ...
    const existingLeads = new Set(db.leads.map((l) => String(l.cvrNummer)));
    let newLeads = 0;
    let hadWebsite = 0;
    let stillUnsure = 0;
    ...
    for (const v of verdicts) {
      ...
      const idx = db.leadVerify.findIndex((l) => String(l.cvrNummer) === cvr);
      if (idx === -1) continue;
...
function saveLeads(results, employee) {
  return mutate((db) => {
    if (!db.leads) db.leads = [];
    const existing = new Set(db.leads.map((l) => String(l.cvrNummer)));
    let added = 0;
    for (const r of results) {
      const cvr = String(r.cvrNummer);
      if (existing.has(cvr)) continue;
      db.leads.push({
        id: nanoid(),
        cvrNummer: cvr,
        navn: r.navn,
