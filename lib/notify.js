...
async function sendSms(phone, text) {
  // Brug GATEWAYAPI_API_KEY fra Railway Variables.
  // Opret konto og API-nøgle på gatewayapi.com — gratis startkredit.
  // Sender vises som firmanavnet (alphanumeric sender ID) på kundens telefon.
  ...
  // Normaliser til MSISDN — kun cifre, med landekode (4512345678 for DK)
  let msisdn = phone.replace(/\s|\+|-/g, "");
  if (msisdn.startsWith("00")) msisdn = msisdn.slice(2);
  if (msisdn.length === 8) msisdn = "45" + msisdn; // dansk nummer uden landekode
  ...
  try {
    const res = await fetch("https://gatewayapi.com/rest/mtsms", {
      ...
      body: JSON.stringify({
        sender: COMPANY_NAME,   // vises som "BuildOne" på kundens telefon
        message: text,
    ...
    console.log(`[SMS] Sendt til ${msisdn} via GatewayAPI`);
    return { sent: true };
  } catch (err) {
