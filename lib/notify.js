...
async function sendSms(phone, text) {
  // Brug TEXTBELT_API_KEY fra Railway Variables.
  // Til test: sæt nøglen til "textbelt" for 1 gratis SMS/dag.
  // Køb credits på textbelt.com/buy — ~0,05 kr/SMS.
  ...
  // Normalize til E.164 format — tilføj +45 hvis dansk nummer uden landekode
  let normalized = phone.replace(/\s/g, "");
  if (!normalized.startsWith("+")) {
    normalized = "+45" + normalized.replace(/^0+/, "");
  }
  ...
  try {
    ...
    if (!data.success) throw new Error(data.error || "Ukendt fejl fra Textbelt");
    console.log(`[SMS] Sendt til ${normalized} — id: ${data.textId}`);
