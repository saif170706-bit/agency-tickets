  ...
  return mutate((db) => {
    ...
    const ticket = {
      ...
      roadmap: ROADMAP_TYPES.includes(type)
        ...
        : [],
      currentStepId: null,
      // Under-trin til "i gang"-fasen (support + vedligeholdelse)
...
function changeStatus(ref, newStatusLabel, employee) {
  return mutate((db) => {
    ...
    if (!ROADMAP_TYPES.includes(ticket.type) && newStatusLabel === "Lukket") {
...
function closeTicket(ref, employee) {
  return mutate((db) => {
    const ticket = db.tickets.find((t) => t.ref === ref);
    if (!ticket) throw new Error("Sag ikke fundet");
    if (!ROADMAP_TYPES.includes(ticket.type)) ticket.statusLabel = "Lukket";
    closeTicketInternal(db, ticket, employee);
    return ticket;
  });
}
