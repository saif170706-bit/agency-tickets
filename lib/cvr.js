...
const SOURCE_FIELDS = [
  ...
  "Vrvirksomhed.hjemmeside",                         // ← CVR's eget URL-felt
...
function buildQuery({
  ...
  if (kunAktive !== false) {
    // Primær check: sammensatStatus skal være NORMAL
    filter.push({ match: { "Vrvirksomhed.virksomhedMetadata.sammensatStatus": "NORMAL" } });

    // Sekundær check: livsforloeb-perioden er stadig åben (ingen gyldigTil)
    // Dette er ERSTs anbefalede metode til at filtrere aktive virksomheder
    filter.push({
      nested: {
    ...
    // Eksplicit ekskluder lukkede/opløste statusser
    mustNot.push({ match: { "Vrvirksomhed.virksomhedMetadata.sammensatStatus": "OPLØST" } });
    mustNot.push({ match: { "Vrvirksomhed.virksomhedMetadata.sammensatStatus": "OPHØRT" } });
    mustNot.push({ match: { "Vrvirksomhed.virksomhedMetadata.sammensatStatus": "UNDER KONKURS" } });
    mustNot.push({ match: { "Vrvirksomhed.virksomhedMetadata.sammensatStatus": "TVANGSOPLØST" } });
  }
...
function parseHit(hit) {
  ...
  // CVR's hjemmeside-felt følger samme historiske array-struktur som telefon/email
  const hjemmeside = pickCurrentContact(v.hjemmeside) || null;
  ...
  return {
    ...
    hjemmeside,           // URL fra CVR — null hvis ikke registreret
...
async function searchCvr(params) {
  ...
  const CLOSED_STATUSES = ["OPLØST", "OPHØRT", "UNDER KONKURS", "TVANGSOPLØST", "LUKKET"];
  return {
    total,
    results: hits
      .map(parseHit)
      .filter(Boolean)
      .filter((r) => !CLOSED_STATUSES.some((s) => (r.status || "").toUpperCase().includes(s))),
  };
}
